...
class RSFormModelForms extends JModelLegacy
{
	...
	function getRequiredFields()
	{
		...
		$this->_db->setQuery("SELECT p.PropertyName, p.PropertyValue, c.ComponentId FROM #__rsform_properties p LEFT JOIN #__rsform_components c ON (p.ComponentId = c.ComponentId) WHERE c.FormId='".$formId."' AND p.PropertyName='NAME' AND c.ComponentTypeId IN (8, 24,124) AND c.Published='1' ORDER BY c.Order");
		$results = $this->_db->loadObjectList();
		foreach ($results as $result) {
			$return[] = $result->PropertyValue;
		}
